...
function initZoomControls() {
  ...
  const defaultSize = 14; // Clean standard base size in px (normal websites)
  let stored = parseFloat(localStorage.getItem('nirikshan_ui_fontsize'));
  
  // Clear legacy bloated sizes or corrupt values
  if (!stored || stored >= 17.0 || stored < 11.0) {
    stored = defaultSize;
    localStorage.removeItem('nirikshan_ui_fontsize');
  }
  let currentSize = stored;
  
  function applySize(size) {
    currentSize = Math.max(11, Math.min(22, Math.round(size * 10) / 10));
    if (Math.abs(currentSize - defaultSize) < 0.2) {
      // Remove inline override so CSS responsive media queries adapt automatically across screens
      document.documentElement.style.fontSize = '';
      localStorage.removeItem('nirikshan_ui_fontsize');
      if (zoomLabel) {
        zoomLabel.textContent = 'Normal';
      }
    } else {
      document.documentElement.style.fontSize = currentSize + 'px';
      localStorage.setItem('nirikshan_ui_fontsize', currentSize);
      if (zoomLabel) {
        const pct = Math.round((currentSize / defaultSize) * 100);
        zoomLabel.textContent = pct + '%';
      }
    }
    if (window.leafletMapInstance && typeof window.leafletMapInstance.invalidateSize === 'function') {
      setTimeout(() => window.leafletMapInstance.invalidateSize(), 150);
    }
  }

  if (zoomInBtn) {
    zoomInBtn.addEventListener('click', () => applySize((currentSize || defaultSize) + 1.0));
  }
  if (zoomOutBtn) {
    zoomOutBtn.addEventListener('click', () => applySize((currentSize || defaultSize) - 1.0));
  }
  if (zoomResetBtn) {
    zoomResetBtn.addEventListener('click', () => applySize(defaultSize));
  }

  if (localStorage.getItem('nirikshan_ui_fontsize')) {
    applySize(currentSize);
  } else {
    document.documentElement.style.fontSize = '';
    if (zoomLabel) zoomLabel.textContent = 'Normal';
  }
}
...
async function initGisDashboard() {
  ...
  const googleRoads = L.tileLayer('https://mt{s}.google.com/vt/lyrs=m&x={x}&y={y}&z={z}', {
    maxZoom: 20,
    subdomains: ['0', '1', '2', '3'],
    attribution: '&copy; Google Maps &bull; Government of Gujarat &bull; Nirikshan'
  }).addTo(leafletMapInstance);
  ...
  const esriStreets = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}', {
    maxZoom: 19,
    attribution: '&copy; Esri World Streets &bull; Government of Gujarat &bull; Nirikshan'
  });

  L.control.layers({
    "🗺️ Google Roadmap": googleRoads,
    "🛰️ Google Satellite": googleSatellite,
    "🏔️ Google Terrain": googleTerrain,
    "🏙️ Esri Streets": esriStreets
  }, null, { position: 'topright' }).addTo(leafletMapInstance);
...
window.clearTrajectoryFromGisMap = function() {
  if (window.activePursuitTrajectories) {
    for (const v of window.activePursuitTrajectories.values()) {
      if (v.layers && leafletMapInstance) {
        v.layers.forEach(l => { try { leafletMapInstance.removeLayer(l); } catch(e){} });
      }
    }
    window.activePursuitTrajectories.clear();
  }
  if (window.trajectoryMapLayers) {
    window.trajectoryMapLayers.forEach(layer => {
      try { leafletMapInstance.removeLayer(layer); } catch(e){}
    });
    window.trajectoryMapLayers = [];
  }
  if (typeof mapTrajectoryLayers !== 'undefined' && Array.isArray(mapTrajectoryLayers)) {
    mapTrajectoryLayers.forEach(layer => {
      try { leafletMapInstance.removeLayer(layer); } catch(e){}
    });
    mapTrajectoryLayers = [];
  }
...
window.markDistrictCamerasCoverageArea = async function(dist) {
  ...
  // 1. Draw prominent dashed boundary outline of the surveillance sector
  if (dFeature) {
  ...
  // 2. Locate all specific cameras installed in this district/cluster
  let allCams = [];
  try {
    if (window.apiClient && typeof window.apiClient.getCameras === 'function') {
      allCams = await window.apiClient.getCameras();
    }
  } catch(e) {
    allCams = window.apiClient?.cameras || [];
  }
  if (!allCams || allCams.length === 0) {
    allCams = window.apiClient?.cameras || [];
  }

  const distKeyword = (dist.name || '').split(' ')[0].toLowerCase().replace(/[^a-z]/g, '');
  const distCams = (allCams || []).filter(c => {
    if (!c.lat || !c.lng) return false;
    if (c.district_id && c.district_id === dist.id) return true;
    if (c.district && c.district.toLowerCase().includes(distKeyword)) return true;
    return false;
  });

  // 3. Smoothly zoom directly to the area where all those cameras are installed
  if (distCams.length === 1) {
    // Single camera: close-up street/node level view
    leafletMapInstance.flyTo([Number(distCams[0].lat), Number(distCams[0].lng)], 14.5, {
      animate: true,
      duration: 1.0
    });
  } else if (distCams.length > 1) {
    // Multiple cameras: fit bounding box encompassing all installed nodes
    const lats = distCams.map(c => Number(c.lat));
    const lngs = distCams.map(c => Number(c.lng));
    const minLat = Math.min(...lats);
    const maxLat = Math.max(...lats);
    const minLng = Math.min(...lngs);
    const maxLng = Math.max(...lngs);

    if (Math.abs(maxLat - minLat) < 0.003 && Math.abs(maxLng - minLng) < 0.003) {
      leafletMapInstance.flyTo([(minLat + maxLat) / 2, (minLng + maxLng) / 2], 14, {
        animate: true,
        duration: 1.0
      });
    } else {
      const camBounds = L.latLngBounds([
        [minLat, minLng],
        [maxLat, maxLng]
      ]);
      leafletMapInstance.flyToBounds(camBounds, {
        padding: [60, 60],
        maxZoom: 15,
        animate: true,
        duration: 1.0
    ...
    // Ensure zoom level unpacks clusters into individual camera pins (zoom >= 11)
    setTimeout(() => {
      if (leafletMapInstance && leafletMapInstance.getZoom() < 10) {
        leafletMapInstance.setZoom(11, { animate: true });
      }
    }, 500);
  } else if (window.activeDistrictDashedPolygon && window.activeDistrictDashedPolygon.getBounds) {
    leafletMapInstance.fitBounds(window.activeDistrictDashedPolygon.getBounds(), { padding: [45, 45], animate: true, duration: 1.0 });
  } else {
    leafletMapInstance.flyTo([dist.lat, dist.lng], 13, { animate: true, duration: 1.0 });
  }

  showRealtimeAlertToast({
    title: `${distLabel.toUpperCase()}: ${distCams.length || dist.total_cams} CAMERAS MONITORED`,
    location: `Zoomed to CCTV installation footprint in ${distLabel}`,
    camera_id: `${dist.coverage_score || 95}% OPERATIONAL`,
    kafka_topic: 'nirikshan.district.focus.grid'
  });
};

window.zoomToDistrictById = function(districtId) {
  if (window.apiClient && window.apiClient.districts) {
    const dist = window.apiClient.districts.find(d => d.id === districtId);
    if (dist) window.markDistrictCamerasCoverageArea(dist);
  }
};
...
async function renderGisNodes(dept = 'ALL', status = 'ALL', search = '') {
  ...
  if (currentZoom < 8.5 && !search && dept === 'ALL' && cameras.length > 30) {
    districts.forEach(dist => {
      ...
      const clusterHtml = `
        <div class="leaflet-cluster-badge" style="width: 44px; height: 44px; cursor: pointer;" onclick="window.zoomToDistrictById('${dist.id}')">
          ${countLabel}
        </div>
      `;
      ...
      // Direct tap/click immediately zooms right to where all the cameras are installed
...
async function renderRegistryTable() {
  ...
  cameras.forEach(cam => {
    ...
    tr.innerHTML = `
      <td><strong style="font-family: var(--font-mono); color: var(--accent-cyan); font-size: 0.88rem;">${cam.id}</strong></td>
      <td>
        <strong style="font-size: 0.9rem; color: #0f172a; display: block; line-height: 1.25; margin-bottom: 2px;">${cam.name}</strong>
        <span style="font-size: 0.76rem; color: var(--text-muted); display: block; line-height: 1.2;">${cam.direction || 'Road View'} &bull; ${cam.lat}, ${cam.lng}</span>
      </td>
      <td><span style="font-weight: 600; font-size: 0.85rem; color: var(--accent-amber); line-height: 1.25; display: inline-block;">${cam.district || 'Ahmedabad'}</span></td>
      <td><span style="font-weight: 600; font-size: 0.84rem;">${getDeptName(cam.department_id)}</span></td>
      <td style="text-align: center;"><span class="node-status-pill ${cam.status}">${cam.status}</span></td>
      <td><span style="font-size: 0.84rem; font-weight: 500; display: block; line-height: 1.2;">${cam.vendor}</span> <span style="font-size: 0.74rem; color: var(--text-muted);">(${cam.type.toUpperCase()})</span></td>
      <td style="text-align: center;"><span style="font-size: 0.84rem; font-weight: 600;">${cam.retention_days}d Buffer</span></td>
      <td style="text-align: center;"><span style="font-family: var(--font-mono); font-weight: 600; font-size: 0.84rem;">${cam.resolution}</span></td>
      <td style="text-align: center;">
        <div style="display: flex; gap: 0.35rem; align-items: center; justify-content: center;">
          <button class="action-btn" onclick="event.stopPropagation(); openCameraDetail('${cam.id}')" title="Inspect Camera Details & FOV" style="padding: 0.3rem 0.55rem; font-size: 0.78rem; font-weight: 600; white-space: nowrap;">
            <i class="fa-solid fa-eye"></i> Details
          </button>
          <button class="action-btn" onclick="event.stopPropagation(); window.deleteCameraById('${cam.id}')" title="Decommission & Remove Node" style="padding: 0.3rem 0.5rem; font-size: 0.78rem; background: rgba(244,63,94,0.15); color: var(--accent-rose); border-color: rgba(244,63,94,0.35);">
...
async function renderLiveWall() {
  ...
  displayCams.forEach((cam, idx) => {
    ...
    cell.addEventListener('click', () => { window.currentActiveLiveCamId = cam.id; });
...
const PURSUIT_COLOR_PALETTE = [
  ...
  { stroke: '#0284c7', glow: '#38bdf8', name: 'Electric Sky Blue' },
  { stroke: '#d97706', glow: '#fbbf24', name: 'Tactical Amber' },
  { stroke: '#7c3aed', glow: '#c084fc', name: 'Cyber Violet' },
  { stroke: '#059669', glow: '#34d399', name: 'Emerald Green' },
  { stroke: '#db2777', glow: '#f472b6', name: 'Hot Magenta' },
  { stroke: '#ea580c', glow: '#fb923c', name: 'Safety Orange' },
  { stroke: '#0891b2', glow: '#22d3ee', name: 'Deep Cyan' }
];

// Map of active pursuit trajectories: normPlate -> { plate, colorConfig, layers, sightings, route, isSuspect, updatedAt }
window.activePursuitTrajectories = new Map();
window.selectedPursuitPlate = null;
window.trajectoryMapLayers = [];

function getVehiclePursuitColor(plateNumber) {
  const norm = (plateNumber || '').replace(/[^A-Z0-9]/g, '');
  if (window.activePursuitTrajectories && window.activePursuitTrajectories.has(norm)) {
    return window.activePursuitTrajectories.get(norm).colorConfig;
  }
  const idx = (window.activePursuitTrajectories ? window.activePursuitTrajectories.size : 0) % PURSUIT_COLOR_PALETTE.length;
  return PURSUIT_COLOR_PALETTE[idx];
}

window.updateMultiVehiclePursuitHud = function() {
  const hud = document.getElementById('pursuitMapHud');
  const btnClear = document.getElementById('btnClearMapPursuit');
  const tabsContainer = document.getElementById('hudVehicleTabs');
  if (!hud) return;

  const count = window.activePursuitTrajectories ? window.activePursuitTrajectories.size : 0;
  if (count === 0) {
  ...
  if (!window.selectedPursuitPlate || !window.activePursuitTrajectories.has(window.selectedPursuitPlate)) {
    window.selectedPursuitPlate = Array.from(window.activePursuitTrajectories.keys())[0];
  }
  ...
  if (tabsContainer) {
    ...
    window.activePursuitTrajectories.forEach((data, normKey) => {
      const isSelected = normKey === window.selectedPursuitPlate;
      const tab = document.createElement('button');
      tab.type = 'button';
      tab.className = `hud-veh-tab ${isSelected ? 'active' : ''}`;
      if (isSelected) {
        tab.style.borderColor = data.colorConfig.stroke;
        tab.style.background = `${data.colorConfig.stroke}18`;
        tab.style.color = data.colorConfig.stroke;
      }
      tab.innerHTML = `
        <span class="tab-dot" style="background: ${data.colorConfig.stroke};"></span>
        <span>${data.plate}</span>
        <span class="tab-close-btn" title="Remove route">&times;</span>
      `;
      tab.addEventListener('click', () => {
        window.selectPursuitVehicle(normKey);
      });
      const closeBtn = tab.querySelector('.tab-close-btn');
      if (closeBtn) {
        closeBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          window.removeVehicleTrajectory(data.plate);
        });
      }
  ...
  const current = window.activePursuitTrajectories.get(window.selectedPursuitPlate);
  if (!current) return;

  const bPlate = document.getElementById('hudPlateBadge');
  if (bPlate) {
    bPlate.textContent = current.plate;
    bPlate.style.color = current.colorConfig.stroke;
  }
  const bVeh = document.getElementById('hudVehicleName');
  if (bVeh) bVeh.textContent = current.sightings[0]?.vehicleType?.toUpperCase() || 'Vehicle';
  const bDist = document.getElementById('hudDistance');
  if (bDist) bDist.textContent = current.route ? `${current.route.distance_km} km` : '0.0 km (Single Node)';
  const bSpeed = document.getElementById('hudSpeed');
  if (bSpeed) bSpeed.textContent = current.route ? `~${current.route.duration_minutes} Mins Drive` : `${((current.sightings[0]?.confidence || 0.95) * 100).toFixed(0)}% Conf`;
  const bHeading = document.getElementById('hudHeading');
  if (bHeading) bHeading.textContent = `${current.sightings[current.sightings.length - 1]?.region || 'Corridor'}`;
  const bNextLoc = document.getElementById('hudNextLoc');
  if (bNextLoc) bNextLoc.textContent = current.route ? `Connected via ${current.route.source || 'Google Maps Roadway Engine'}` : 'Single Checkpoint logged.';
  const bEta = document.getElementById('hudEta');
  if (bEta) bEta.textContent = current.route ? `Transit ETA: ~${current.route.duration_minutes} Mins` : 'Single Node';

  const btnGmaps = document.getElementById('btnOpenGoogleMapsHud');
  if (btnGmaps) {
    if (current.route && current.route.google_maps_url) {
      btnGmaps.href = current.route.google_maps_url;
...
window.selectPursuitVehicle = function(normKey) {
  if (!window.activePursuitTrajectories || !window.activePursuitTrajectories.has(normKey)) return;
  window.selectedPursuitPlate = normKey;
  const current = window.activePursuitTrajectories.get(normKey);
  window.updateMultiVehiclePursuitHud();
  if (leafletMapInstance && current && current.sightings && current.sightings.length > 0) {
    if (current.sightings.length === 1) {
      leafletMapInstance.setView([current.sightings[0].latitude, current.sightings[0].longitude], 14);
    } else {
      const geometry = current.route?.route_geometry || current.sightings.map(s => [s.latitude, s.longitude]);
...
window.renderTrajectoryOnGisMap = async function(plateNumber, shouldSwitchView = true, isSuspect = false, focusOnThis = true) {
  ...
  const mapInput = document.getElementById('mapPursuitInput');
  if (mapInput && shouldSwitchView) mapInput.value = cleanPlate;
  ...
  // Switch to Dashboard GIS Map View and scroll into view if user initiated
  if (shouldSwitchView) {
    const dashNavBtn = document.querySelector('.main-nav-btn[data-view="view-dashboard"]');
    if (dashNavBtn) {
      dashNavBtn.click();
    }
    const mapCard = document.getElementById('leafletMap') || document.querySelector('.gis-map-card') || document.getElementById('gisMapCard');
    if (mapCard) {
      setTimeout(() => {
  ...
  const refreshMapDisplay = () => {
    if (leafletMapInstance) {
      leafletMapInstance.invalidateSize();
    }
  };
  setTimeout(refreshMapDisplay, 80);
  setTimeout(refreshMapDisplay, 250);
  setTimeout(refreshMapDisplay, 500);
  ...
  // Remove previous layers ONLY for this specific vehicle so other vehicles remain visible!
  const oldEntry = window.activePursuitTrajectories.get(normPlate);
  if (oldEntry && oldEntry.layers) {
    oldEntry.layers.forEach(l => {
      try { leafletMapInstance.removeLayer(l); } catch(e){}
    });
  }

  // Get distinct color assigned to this vehicle
  const colorConfig = getVehiclePursuitColor(cleanPlate);
  const vehicleLayers = [];
  ...
  if (traj.status === 'single_point' || traj.sightings.length === 1) {
    ...
    const googleMapsUrl = route.google_maps_url || (() => {
      const origin = `${traj.sightings[0].latitude},${traj.sightings[0].longitude}`;
      const dest = `${traj.sightings[traj.sightings.length - 1].latitude},${traj.sightings[traj.sightings.length - 1].longitude}`;
      const waypoints = traj.sightings.length > 2 ? traj.sightings.slice(1, -1).map(s => `${s.latitude},${s.longitude}`).join('|') : '';
      return `https://www.google.com/maps/dir/?api=1&origin=${origin}&destination=${dest}${waypoints ? `&waypoints=${encodeURIComponent(waypoints)}` : ''}&travelmode=driving`;
    })();

    // 1. Draw Google Maps Styled Road Navigation Route in Distinct Color
    const roadGlow = L.polyline(geometry, {
  ...
  // Save to multi-vehicle pursuit map
  window.activePursuitTrajectories.set(normPlate, {
    plate: cleanPlate,
    colorConfig,
  ...
  if (focusOnThis) {
    window.selectedPursuitPlate = normPlate;
  }

  // Sync legacy layers array
  window.trajectoryMapLayers = [];
  for (const item of window.activePursuitTrajectories.values()) {
    window.trajectoryMapLayers.push(...item.layers);
  }
  ...
  if (shouldSwitchView) {
    showRealtimeAlertToast({
      title: `🗺️ PURSUIT ROUTE: ${cleanPlate}`,
      location: `${traj.sightings.length} Checkpoints • Color: ${colorConfig.name}`,
...
function initDynamicIntelligence() {
  ...
  if (window.apiClient && window.apiClient.connectDetectionStream) {
    window.apiClient.connectDetectionStream(async (eventType, payload) => {
      if (eventType === 'new_detection') {
        ...
        const vId = (payload.vehicleId || payload.plate || '').trim().toUpperCase();
        const isSuspect = payload.is_suspect || (payload.suspect_match && payload.suspect_match.status === 'MATCH');

        // Real-Time Alert Section notification when suspect vehicle is detected
        if (isSuspect) {
          await renderAlerts();
          updateGlobalAlertBadge();
          window.playTacticalAlertSiren();

          const camName = payload.cameraName || payload.cameraId || 'Surveillance Node';
          const stationInfo = window.getNearestPoliceStation({ id: payload.cameraId, district: payload.region, name: camName });

          window.showTacticalBoloToast({
            plate: vId,
            crime: payload.suspect_match?.suspect?.crime || 'Active Investigative Warrant',
            cam_name: camName,
        ...
        // ONLY trigger real-time GIS route drawing for:
        // 1. Vehicles verified as suspect matches against active watchlist (isSuspect === true)
        // 2. OR vehicles currently being actively tracked on the map (isPursuitTarget || isAlreadyTracked)
        if (vId && vId !== 'OCR UNRESOLVED') {
          const pursuitInput = document.getElementById('mapPursuitInput');
          const isPursuitTarget = pursuitInput && pursuitInput.value && pursuitInput.value.trim().toUpperCase() === vId;
          const isAlreadyTracked = window.activePursuitTrajectories && window.activePursuitTrajectories.has(vId.replace(/[^A-Z0-9]/g, ''));

          if (isSuspect || isPursuitTarget || isAlreadyTracked) {
            console.log(`[REAL-TIME-PURSUIT] Target vehicle ${vId} sighted on camera grid. Updating GIS route simultaneously...`);
            window.renderTrajectoryOnGisMap(vId, false, isSuspect, isSuspect);
          }
        }
      } else if (eventType === 'new_alert') {
        if (!window.apiClient.alerts) window.apiClient.alerts = [];
        if (!window.apiClient.alerts.some(a => a.id === payload.id)) {
          window.apiClient.alerts.unshift(payload);
        }
        await renderAlerts();
        updateGlobalAlertBadge();
        window.playTacticalAlertSiren();

        if (payload.target_vehicle) {
          window.renderTrajectoryOnGisMap(payload.target_vehicle, false, true, true);
        }
      } else if (eventType === 'alerts_updated') {
        renderAlerts();
  ...
  if (btnToggleForm && watchForm) {
    ...
    window.populateTargetRegCameraOptions = function(cameras) {
      ...
      const targetCam = window.currentActiveLiveCamId || 'cam32';
      select.innerHTML = '<option value="AUTO">🔴 Live CCTV Dynamic Scan (Auto Intercept)</option>' +
        cameras.map(c => `<option value="${c.id}" ${c.id.toLowerCase() === targetCam.toLowerCase() ? 'selected' : ''}>[${c.id.toUpperCase()}] ${c.district.split('(')[0].trim()} • ${c.name}</option>`).join('');
    ...
    watchForm.addEventListener('submit', async (e) => {
      ...
      let selectedCam = (cameraInput?.value || 'AUTO');
      if (selectedCam === 'AUTO' && window.currentActiveLiveCamId) {
        selectedCam = window.currentActiveLiveCamId;
      }
      if (selectedCam === 'AUTO') {
        selectedCam = 'cam32';
      }
...
window.triggerLiveCctvSuspectDetection = async function(payload) {
  ...
  // Automatically send alert near authority and simultaneously open live track on map!
  setTimeout(() => {
    window.renderTrajectoryOnGisMap(plate, true);
  }, 400);
};
...
window.showTacticalBoloToast = function(data) {
  ...
  toast.innerHTML = `
    ...
    <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
      <button type="button" class="action-btn" onclick="window.renderTrajectoryOnGisMap('${data.plate}', true); document.getElementById('tacticalBoloToast')?.remove();" style="font-size: 0.74rem; padding: 0.35rem 0.75rem; background: rgba(0, 242, 254, 0.2); border: 1px solid #00f2fe; color: #00f2fe; font-weight: 700;">
      ...
      <button type="button" class="action-btn primary" onclick="document.querySelector('[data-view=\\'view-alerts\\']')?.click(); document.getElementById('tacticalBoloToast')?.remove();" style="font-size: 0.74rem; padding: 0.35rem 0.75rem; background: #dc2626; border-color: #dc2626; font-weight: 700;">
        <i class="fa-solid fa-bell"></i> Alerts Section
      </button>
    </div>
  `;
...
async function renderSuspectMatches() {
  ...
  if (!suspects || suspects.length === 0) {
    container.innerHTML = `
      <div class="empty-suspect-notice" style="display: flex; align-items: center; justify-content: center; gap: 0.65rem; padding: 0.75rem 1.2rem; background: #f8fafc; border: 1px dashed #cbd5e1; border-radius: 6px; color: #475569;">
        <i class="fa-solid fa-shield-halved" style="font-size: 1.05rem; color: #94a3b8;"></i>
        <div style="text-align: left;">
          <strong style="color: #0f172a; font-size: 0.82rem; font-weight: 700;">No suspect match found.</strong>
          <span style="font-size: 0.72rem; color: #64748b; margin-left: 0.4rem;">Vehicles only appear here upon an authorized watchlist database match.</span>
        </div>
      </div>
    `;
    return;
  ...
  suspects.forEach(s => {
    ...
    const statusPill = isVerified
      ? `<span class="node-status-pill offline" style="background: rgba(239,68,68,0.12); color: #dc2626; border: 1px solid #ef4444;"><i class="fa-solid fa-triangle-exclamation"></i> VERIFIED SUSPECT MATCH</span>`
      : `<span class="node-status-pill warning" style="background: rgba(245,158,11,0.12); color: #d97706; border: 1px solid #f59e0b;"><i class="fa-solid fa-circle-question"></i> Potential match — verification required</span>`;

    card.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.4rem;">
        ${statusPill}
        <strong style="font-family: var(--font-mono); color: #0284c7; font-size: 0.95rem;">${targetPlate}</strong>
      </div>
      <div style="font-size: 0.82rem; font-weight: 700; color: #0f172a; margin-bottom: 0.25rem;">
        ${s.suspect_match?.suspect?.crime || s.suspect_match?.message || 'Watchlist Hit'}
      </div>
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 0.4rem; font-size: 0.72rem; color: #64748b; margin-bottom: 0.6rem;">
        <div><span>Camera:</span> <strong style="color: #0f172a;">${s.cameraName}</strong></div>
        <div><span>Region:</span> <strong style="color: #0f172a;">${s.region}</strong></div>
        <div><span>Time:</span> <strong style="color: #0f172a;">${new Date(s.timestamp).toLocaleTimeString()}</strong></div>
        <div><span>Match Conf:</span> <strong style="color: ${isVerified ? '#dc2626' : '#d97706'};">${s.suspect_match?.confidence || 99.4}%</strong></div>
      </div>
      <div style="display: flex; gap: 0.4rem; flex-wrap: wrap;">
        <button type="button" class="action-btn primary" onclick="window.renderTrajectoryOnGisMap('${targetPlate}', true)" style="font-size: 0.72rem; padding: 0.3rem 0.6rem;">
          <i class="fa-solid fa-route"></i> Trace Route on Map
        </button>
        <button type="button" class="action-btn" onclick="window.openSuspectSightingCctv('${targetPlate}', '${s.cameraId}')" style="font-size: 0.72rem; padding: 0.3rem 0.6rem; background: #ffffff; color: #0f172a; border: 1px solid #cbd5e1;">
          <i class="fa-solid fa-video"></i> View Sighting Feed
        </button>
        <button type="button" class="action-btn danger btn-resolve-suspect" data-plate="${targetPlate}" style="font-size: 0.72rem; padding: 0.3rem 0.6rem; background: rgba(239, 68, 68, 0.1); border: 1px solid #ef4444; color: #dc2626; cursor: pointer; transition: all 0.2s ease;" title="Mark problem solved & remove this vehicle from suspect records">
...
async function renderAnalyticsTable() {
  ...
  filtered.forEach(d => {
    ...
    const isPlateResolved = d.plate && d.plate !== 'OCR UNRESOLVED' && !d.plate.includes('UNRESOLVED');
    let enhancerBadge = '';
    if (d.enhancement_applied) {
      enhancerBadge = isPlateResolved
        ? `<span style="font-size: 0.65rem; background: rgba(16, 185, 129, 0.2); color: #10b981; border: 1px solid #10b981; border-radius: 3px; padding: 1px 5px; margin-left: 5px; font-weight: 600; letter-spacing: 0.4px;" title="Quality restored via DIP Theorem & Color Grading Enhancer"><i class="fa-solid fa-wand-magic-sparkles"></i> DIP ENHANCED</span>`
        : `<span style="font-size: 0.65rem; background: rgba(245, 158, 11, 0.15); color: #f59e0b; border: 1px solid rgba(245, 158, 11, 0.4); border-radius: 3px; padding: 1px 5px; margin-left: 5px; font-weight: 600; letter-spacing: 0.4px;" title="Low optical resolution: DIP theorem & color grading enhancer applied but characters below legibility threshold"><i class="fa-solid fa-microchip"></i> DIP APPLIED</span>`;
    } else if (isPlateResolved) {
      enhancerBadge = `<span style="font-size: 0.65rem; background: rgba(56, 189, 248, 0.15); color: #38bdf8; border: 1px solid rgba(56, 189, 248, 0.4); border-radius: 3px; padding: 1px 5px; margin-left: 5px; font-weight: 600; letter-spacing: 0.4px;" title="Direct optical read from high quality CCTV frame"><i class="fa-solid fa-check"></i> HIGH RES</span>`;
    }

    const plateHtml = isPlateResolved
    ...
    tr.innerHTML = `
      ...
      <td>
        <div style="display: flex; gap: 0.3rem;">
          ...
          <button type="button" class="action-btn" style="font-size: 0.7rem; padding: 0.2rem 0.45rem;" onclick="window.renderTrajectoryOnGisMap('${d.vehicleId}', true)" title="Trace Dynamic Road Route">
            <i class="fa-solid fa-route"></i> Route
          </button>
...
window.openEvidentiarySnapshotModal = async function(detectionId, camId) {
  ...
  let activePlate = existingDet?.plate || existingDet?.vehicleId || 'OCR UNRESOLVED';
  ...
  function stylePlateDisplay(plateStr) {
    ...
    if (plateStr && plateStr !== 'OCR UNRESOLVED' && !plateStr.includes('UNRESOLVED')) {
      evPlateText.textContent = plateStr;
      evPlateText.style.color = 'var(--accent-cyan)';
    } else {
      evPlateText.textContent = 'OCR UNRESOLVED';
      evPlateText.style.color = '#f59e0b';
    }
  ...
  const spinnerAutoTimer = setTimeout(() => {
    if (evLoadingSpinner && evLoadingSpinner.style.display !== 'none') {
      evLoadingSpinner.style.display = 'none';
    }
  }, 2500);
  ...
  function updateCropDisplay() {
    if (!evCropImg) return;
    if (!activeVeh) {
      if (evCropPlateReadout) evCropPlateReadout.textContent = activePlate || 'MONITORING ACTIVE TRAFFIC';
      if (evCropVisBadge) evCropVisBadge.textContent = 'OPTICAL SENSOR';
      if (evFullFrameImg && evFullFrameImg.src && evFullFrameImg.src.length > 20) {
        evCropImg.src = evFullFrameImg.src;
      }
      return;
    }
    const targetUrl = (isEnhanced && activeVeh.enhanced_crop_url) ? activeVeh.enhanced_crop_url : (activeVeh.crop_url || evFullFrameImg?.src);
    ...
    btnPull.onclick = async () => {
      ...
      try {
        ...
        if (fresh && fresh.status === 'success') {
          ...
          const rawUrl = fresh.full_frame_url || fresh.snapshot_url || '';
          const safeFull = rawUrl.startsWith('data:') ? rawUrl : (rawUrl ? (rawUrl + (rawUrl.includes('?') ? '&' : '?') + `t=${Date.now()}`) : '');
          if (evFullFrameImg && safeFull) {
            evFullFrameImg.src = safeFull;
            if (evLoadingSpinner) evLoadingSpinner.style.display = 'none';
          }
          if (evTimestampText) {
  ...
  (async function loadInitialSnapshot() {
    try {
      ...
      if (snapshotData && snapshotData.status === 'success') {
        ...
        const safeFull = fullUrl.startsWith('data:') ? fullUrl : (fullUrl ? (fullUrl + (fullUrl.includes('?') ? '&' : '?') + `t=${Date.now()}`) : '');
        const cropUrl = snapshotData.crop_url || snapshotData.enhanced_crop_url || '';
        const safeCrop = cropUrl.startsWith('data:') ? cropUrl : (cropUrl ? (cropUrl + (cropUrl.includes('?') ? '&' : '?') + `t=${Date.now()}`) : '');
        ...
        if (snapshotData.vehicles && snapshotData.vehicles.length > 0) {
          ...
          renderVehicleChips([]);
          activeVeh = null;
          if (evPlateText) evPlateText.textContent = snapshotData.plate || activePlate || 'MONITORING ACTIVE TRAFFIC';
          if (evCropBadge) evCropBadge.textContent = 'MONITORING';
          if (evOcrStatusText) evOcrStatusText.textContent = 'OPTICAL FEED';
          if (evCropImg) {
            evCropImg.src = (safeCrop && safeCrop.length > 20) ? safeCrop : (safeFull || '');
          }
        }
        updateCropDisplay();
      ...
      <div class="alert-card-footer">
        ...
        <div style="display: flex; gap: 0.4rem; flex-wrap: wrap;">
          ...
          ${alert.target_vehicle ? `
            <button class="action-btn" onclick="window.renderTrajectoryOnGisMap('${alert.target_vehicle}', true)" style="background: rgba(0,242,254,0.15); border: 1px solid #00f2fe; color: #00f2fe; font-size: 0.72rem; padding: 0.3rem 0.65rem; font-weight: 700;" title="Trace Multi-Dept Pursuit Route on GIS Map">
              <i class="fa-solid fa-map-location-dot"></i> Track on GIS
            </button>
          ` : ''}
          ${alert.status === 'active' ? `
...
// Action Handlers
window.alertNearestPoliceStation = async function(alertId) {
  const alertItem = (window.apiClient.alerts || []).find(a => a.id === alertId);
  const stName = alertItem?.assigned_station || 'District Police Station';
  const pcr = alertItem?.pcr_unit || 'PCR Interceptor Unit';
  const eta = alertItem?.eta || '1.8 mins';
  const barrier = alertItem?.forward_roadblock_location || 'Forward Roadblock Checkpost';
  const radio = alertItem?.radio_grid || 'APCO-25 Secure VHF Grid';
  const phone = alertItem?.police_phone || 'Emergency 112';
  ...
  try {
    fetch(`/api/alerts/${encodeURIComponent(alertId)}/dispatch`, { method: 'POST' });
  } catch(e){}
  ...
  showRealtimeAlertToast({
    title: `🚓 POLICE STATION ALERTED: ${stName.split('&')[0].trim()}`,
    location: `${pcr} (ETA ${eta}) deployed. Roadblock ${barrier} armed. Radio: ${radio}. Phone: ${phone}.`,
    camera_id: alertItem?.camera_id || 'DISPATCH_HQ'
  });
};

window.alertControlRoomAuthority = async function(alertId) {
  const alertItem = (window.apiClient.alerts || []).find(a => a.id === alertId);
  const targetPlate = alertItem?.target_vehicle || 'SUSPECT VEHICLE';
  const loc = alertItem?.location || 'State Highway Intercept Grid';

  window.playTacticalAlertSiren();
  showRealtimeAlertToast({
    title: `🚨 STATE CONTROL ROOM (112) BROADCAST`,
    location: `Emergency priority BOLO active for ${targetPlate} across all highway patrol squads. Tactical intercept armed at ${loc}.`,
    camera_id: 'CONTROL_112'
  });
};
...
// =========================================================================
// NIRIKSHAN / CITY MONITORING LOGIN GATEWAY CONTROLLER
// =========================================================================
function initLoginAuth() {
  ...
  const headerOperatorId = document.getElementById('headerOperatorId');
  const lockConsoleBtn = document.getElementById('lockConsoleBtn');

  function checkSession() {
    try {
      const session = localStorage.getItem('nirikshan_session') || sessionStorage.getItem('nirikshan_session');
      if (session) {
        const user = JSON.parse(session);
        if (loginScreen) loginScreen.style.display = 'none';
        if (appLayout) appLayout.style.display = 'flex';
        if (headerOperatorId && user.badgeNumber) {
          headerOperatorId.textContent = user.badgeNumber;
        }
  ...
  // Forgot Password / Quick Demo Autofill
  if (loginForgotBtn) {
    loginForgotBtn.addEventListener('click', () => {
      if (loginUsername) loginUsername.value = 'POLICE-SURV-101';
      if (loginPassword) loginPassword.value = 'Nirikshan2026';
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', (e) => {
      ...
      if (loginSubmitBtn) {
        ...
        const btnArrow = loginSubmitBtn.querySelector('.btn-arrow');
        const spinner = loginSubmitBtn.querySelector('.login-spinner');
      ...
      setTimeout(() => {
        const sessionUser = {
          username: u,
          name: u.includes(' ') ? u : `Officer ${u}`,
          badgeNumber: u.toUpperCase(),
          role: 'City Surveillance Operator',
          authenticatedAt: new Date().toISOString()
        };

        const remember = document.getElementById('loginRememberMe');
        if (remember && remember.checked) {
          localStorage.setItem('nirikshan_session', JSON.stringify(sessionUser));
        } else {
          sessionStorage.setItem('nirikshan_session', JSON.stringify(sessionUser));
        }

        if (headerOperatorId) headerOperatorId.textContent = sessionUser.badgeNumber;
        if (loginScreen) loginScreen.style.display = 'none';
        if (appLayout) appLayout.style.display = 'flex';
        ...
        // Trigger resize for Leaflet GIS maps and layout redraw
        window.dispatchEvent(new Event('resize'));
        if (typeof renderGisMap === 'function') renderGisMap();
      }, 400);
    });
  }

  // Lock Console button in header
  if (lockConsoleBtn) {
    lockConsoleBtn.addEventListener('click', () => {
      localStorage.removeItem('nirikshan_session');
      sessionStorage.removeItem('nirikshan_session');
      if (appLayout) appLayout.style.display = 'none';
      if (loginScreen) {
        loginScreen.style.display = 'flex';
        const uField = document.getElementById('loginUsername');
        const pField = document.getElementById('loginPassword');
        if (uField && !uField.value) uField.value = 'POLICE-SURV-101';
        if (pField && !pField.value) pField.value = 'Nirikshan2026';
      }
    });
  }
...
// =========================================================================
// BIOMETRIC FACIAL DETECTION API HOOK (POWERED BY models/ FOLDER)
// =========================================================================
window.runFacialDetectionWithModels = async function(cameraId = 'cam08') {
  try {
    const res = await fetch(`/api/facial/detect?camera_id=${encodeURIComponent(cameraId)}`);
    const data = await res.json();
    console.log('[MODELS-FACIAL-ENGINE] Detection Results:', data);
    return data;
  } catch (err) {
    console.error('[MODELS-FACIAL-ENGINE] Detection failed:', err);
    return null;
  }
};
