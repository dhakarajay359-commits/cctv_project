...
def extract_license_plate_crop(veh_crop: np.ndarray, vehicle_type: str = "car") -> np.ndarray:
    ...
    if candidates:
        ...
        if plate_crop.shape[0] >= 6 and plate_crop.shape[1] >= 14:
            return plate_crop
    ...
    for c in contours_e:
        ...
        aspect = cw / float(max(1, ch))
        area = cw * ch
        if 1.8 <= aspect <= 5.5 and (rw * 0.12 <= cw <= rw * 0.40) and (ch >= max(6, int(rh * 0.05)) and ch <= rh * 0.25):
            center_x = cx + cw / 2.0
    ...
    # Tight canonical license plate focus fallback (strictly rectangular plate slot, NEVER full bumper)
    pw = max(20, min(int(vw * 0.25), 110))
    ph = max(7, int(pw / 3.4))
    cx = vw // 2
    if is_truck_bus:
        cy = int(vh * 0.80)
