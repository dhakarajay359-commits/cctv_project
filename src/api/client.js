    ...
    try {
      ...
      es.addEventListener('new_alert', (e) => {
        try {
          ...
          if (!this.alerts.some(a => a.id === alertData.id)) {
            this.alerts.unshift(alertData);
          }
          if (callback) callback('new_alert', alertData);
        } catch(err) {}
      });
      es.addEventListener('alerts_updated', (e) => {
        try {
          const a = JSON.parse(e.data);
          if (callback) callback('alerts_updated', a);
        } catch(err) {}
      });
